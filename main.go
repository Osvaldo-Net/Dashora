...
// parseRSSFeed intenta primero formato RSS estándar y luego Atom como fallback.
func parseRSSFeed(body []byte, maxEntries int) ([]RSSItemJSON, error) {
	// ── Intento 1: RSS estándar ──────────────────────────────────────────
	var feed rssXML
	...
	for _, e := range entries {
		// Prefiere el link con rel="alternate"; si no, el primero disponible
		link := ""
		...
		// Fallback: usar el ID de Atom (suele ser la URL del post)
		if link == "" {
...
// newRSSClient devuelve un http.Client con los headers necesarios para que
// sitios como Reddit no rechacen la petición.
...
func fetchRSSFeed(w http.ResponseWriter, r *http.Request) {
	...
	// ── Petición HTTP con User-Agent y Accept correctos ──────────────────
